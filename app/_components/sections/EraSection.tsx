  ...
  useEffect(() => {
    async function fetchData() {
      ...
      for (const startYear of eras) {
        ...
        // Collect unique artists (first artist per album)
        ...
        const selected = Array.from(artistMap.values()).slice(
          0,
          targetArtistsPerEra
        );
        ...
        const images = selected
          .map(a => a.image)
          .filter(Boolean) as UpToFour<MediaImage>;
        ...
        results.push({
          ...
          meta: (
            <span className="text-xs opacity-70 line-clamp-2">
              ...
              {artistMap.size > selected.length && (
